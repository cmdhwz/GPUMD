...
void NEP::initialize_pimd_batch_(
...
static __global__ void initialize_nep_pimd_batch_properties(
  const int N,
...
static __global__ void find_neighbor_list_large_box_nep_pimd_batch(
  NEP::ParaMB paramb,
  const int N,
  const int N1,
  const int N2,
...
static __global__ void find_descriptor_nep_pimd_batch(
  NEP::ParaMB paramb,
  NEP::ANN annmb,
  const int N,
  const int N1,
...
static __global__ void find_force_radial_nep_pimd_batch(
  NEP::ParaMB paramb,
  NEP::ANN annmb,
  const int N,
  const int N1,
  ...
  double* const* g_virial)
{
  ...
  for (int i1 = 0; i1 < g_NN[n1]; ++i1) {
    ...
    for (int n = 0; n <= paramb.n_max_radial; ++n) {
      ...
      const float tmp21 = g_Fp[n2 + n * N] * gnp21 * d12inv;
      for (int d = 0; d < 3; ++d) {
        f12[d] += tmp12 * r12[d];
        f21[d] -= tmp21 * r12[d];
      }
...
static __global__ void find_partial_force_angular_nep_pimd_batch(
  NEP::ParaMB paramb,
  NEP::ANN annmb,
  const int N,
  const int N1,
...
static __global__ void find_force_many_body_nep_pimd_batch(
  NEP::ParaMB paramb,
  const int N,
  const int N1,
  const int N2,
  ...
  const std::vector<GPU_Vector<double>*>& virial_beads)
{
  const int number_of_beads = int(position_beads.size());
  if (
    paramb.model_type != 0 || number_of_beads < 2 ||
